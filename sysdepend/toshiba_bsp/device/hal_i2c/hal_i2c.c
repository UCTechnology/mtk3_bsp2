...
void INTI2C1_IRQHandler(void)
{
	...
	for(i = 0; i < DEV_HAL_I2C_UNITNM; i++) {
		p_dcb = get_dcb_ptr(i);
                if(EI2C_status_spcf(&p_dcb->hi2c->ei2c)) {
                        EI2C_clear_int_status(&p_dcb->hi2c->ei2c);
			p_dcb->err = E_OK;
...
LOCAL ER read_data(T_HAL_I2C_DCB *p_dcb, T_DEVREQ *req)
{
	...
	case HAL_I2C_MODE_CNT:
		err = dev_i2c_master_receive(
			p_dcb->hi2c,	// I2C_Handle
                        req->start,
			(UB*)req->buf,		// Pointer to data buffer
 ...
 */
EXPORT ER dev_init_hal_i2c( UW unit, bsp_ei2c_t *hi2c )
{
	T_HAL_I2C_DCB	*p_dcb;
	T_IDEV		idev;
	T_MSDI		*p_msdi;
...
EXPORT ER i2c_read_reg(ID dd, UW sadr, UW radr, UB *data)
{
	...
	err = dev_i2c_master_receive_reg(
		p_dcb->hi2c,	// I2C_Handle
		sadr,
		radr,		// Pointer to data buffer
		data,
